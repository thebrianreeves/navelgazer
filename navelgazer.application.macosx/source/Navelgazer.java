...
public class Navelgazer extends PApplet {
...
float magicRandomLow = 1.8f;
float magicRandomHigh = 5.7f; 
...
float initialMagicNumber  =  3.226f;   // was 2.618;
float magicNumber;
int whichMark = 0;
float dotDistance; 
...
public void setup (){ 
   ...
   shapeMode(CENTER);
    ...
    for (int c =1;c<1000;c+=20) {
      ...
      ellipse (width/2,height/2+210,magicNumber*1+c*2.3f,magicNumber*300+c*.618f);
    }
    ...
    for (int c =1;c<1000;c+=60) {
       ...
       strokeWeight(.8f);
       stroke(40,60,155,3);
       ellipse (width/2,height/2+250,magicNumber*1-c*2.1f,magicNumber*300+c*.8f);
    }
   ...
   for (int c =1;c<1000;c+=30) {
       ...
       strokeWeight(2);
       stroke(255,120,80,4);
       ellipse (width/2,height/2+190+c,magicNumber*1+c*2.3f,magicNumber*300+c*.618f);
     }
    ...
    for (int c =1;c<1000;c+=30) {
        ...
        strokeWeight(.8f);
        stroke(40,60,155,10);
        ellipse (width/2+width+1000-c,height/2+220,magicNumber*1-c*2.1f,magicNumber*300+c*.8f);
      }
...
public void draw () { 
     ...
     if (randomMode > 0 ) {
         ...
         magicNumber = random(2.618f,4.5f);
      ...
      if (hairMode == 1 ){
         ...
         float distanceFromCenter = dist (rXbegin+(rLength/1.618f),rYbegin+(rLength/2.6f),width/2,height/magicNumber);

         // if (the distance between x and y) and the center sketchWidth/2,sketchHeight/3
         if (distanceFromCenter > navelSize/2.4f) {
...
public void determineWhichMark() {
...
public void drawControlPanel() {
    ...
    textSize(24);
    textAlign(LEFT);
    text("Brian Reeves",59,controlPanelHeight+29.5f);
    ...
    textFont(arial);
    textAlign(LEFT);
    text("American 1969-",219,controlPanelHeight+29.5f);
  
    textSize(7);
  
    textFont(arial);
    textAlign(LEFT);
    text("1920 x 1052 pixels, 24 bit color, written using Processing",59,controlPanelHeight+86.5f);
    ...
    textFont(KenyanFont);
    textAlign(LEFT);
    text("Automated Navelgazing",57,controlPanelHeight+62.5f);
  ...
  // magic number control
      ...
      fill(20,100);
      ellipse(masterstrokeSecondLineX+406,masterstrokeSecondLineY+22-(thisMagicNumber*8.667f),6,6); 
      
      fill (210,140,23);
      ellipse(masterstrokeSecondLineX+406,masterstrokeSecondLineY+23-(thisMagicNumber*8.667f),6,6); 
      fill (40);
      textFont (arial);
      textSize (14);
...
public void drawMarkMarkersAndLabels() {
    if (whichMark == 1) {
        theMarkThumbnail = loadImage ("NAVEL MARKS_button_1.jpg");
    }
...
public void mark1() {
  ...
  whichMark = 1;
    ...
    for (int i = 0;i<1*amplification*thisMarkIntensity*1;i++){
      ...
      float speed = random (.02f);
      float angle = 0;
      float arcX = width/2;
      float arcY = height/thisMagicNumber;
      float arcSize = width/4.618f+variation;
...
public void mark2()  {
    ...
    if (randomMode == 1 )  {
          thisMarkIntensity = random(0,100);
          thisMagicNumber = random (2.1f,magicRandomHigh);
    ...
    for (int i = 0;i<1*amplification*thisMarkIntensity*1;i++)  {
         ...
         float arcSize = width/5.618f+variation;
    ...
    //thisMagicNumber = markMagicNumberArray[2]/1000;
      ...
      if (randomMode == 1 )  {
       ...
       for (int i = 0;i<amplification*thisMarkIntensity*1;i++){   
           float startRadian = random (6.28f);
           float endRadian = startRadian + random (6.28f);
           ...
           float angle = 0;
           float arcX = width/2;
           float arcY = height/(thisMagicNumber+.618f);
           float arcSize = width/1.8f+variation;
...
public void mark4() {
    ...
    // thisMagicNumber = markMagicNumberArray[3]/1000;
     if (randomMode == 1 )  {
        ...
        thisMagicNumber = random (3.5f,magicRandomHigh);
     ...
     for (int i = 0;i<1*amplification*thisMarkIntensity*1;i++)  {
          ...
          float arcX = width/2;
          float arcY = height/thisMagicNumber;
          float arcSize = width/3.4f+variation;
          stroke (random(195,255),random(190,225),random(140,170),random(5));
          float Xoffset = random (-width/1.7f,width/1.7f); 
          float Yoffset = random (-height/1.9f,height/1.9f); 
          float x = arcX + Xoffset + cos(angle) * scalar;
          float y = arcY + Yoffset + sin(angle) * scalar;
          ellipse (x, y, arcSize, arcSize);
          angle += speed;
     ...
     if (randomMode == 1 )  {
      ...
      markIntensityArray[4] = (int)thisMarkIntensity;
       ...
       for (int x = 1;x<amplification*thisMarkIntensity;x++) { 
         float crevasse1xBegin = width/2  - (navelSize - random (navelSize/2-navelSize/1.1f,navelSize/1.3f));
         float crevasse1yBegin = height/2 - random (navelSize/thisMagicNumber,navelSize+thisMagicNumber*5);
         float crevasse1xEnd = (width/2) + (navelSize -random (navelSize/2-navelSize/1.6f,navelSize/1.3f));
       ...
       for (int x = 1;x<amplification*500;x++) { 
         float crevasse1xBegin = width/2  - (navelSize - random (navelSize/2-navelSize/1.6f,navelSize/1.3f));
...
public void mark6() {
      ...
      for (int x = 0;x<amplification*thisMarkIntensity*1;x++) {
           ...
           float crevasse1yBegin = height/2 + height/thisMagicNumber - random (navelSize/(thisMagicNumber*1.4f),navelSize/(thisMagicNumber*.4f));
...
public void mark8() {
   ...
   if (randomMode == 1 )  {
    ...
    for (int i = 0;i<amplification*thisMarkIntensity;i++)  {
       ...
       strokeWeight (random(.5f,variation*(thisMagicNumber)));
       float scalar = random (1,2);
       float speed = random (.02f);
       float angle = 0;   
       float arcX = width/2+thisMagicNumber/3;
       float arcY = height/2-thisMagicNumber*10;
       float arcSize = width/(random(1.2f+thisMagicNumber));
   ...
   if (makeThumbnails == 1 )  {
      ...
      for (int i = 0;i<amplification*300;i++)  {
             ...
             float arcX = width/2+thisMagicNumber/3;
             float arcY = height/2-thisMagicNumber*10;
             float arcSize = width/2.98f+variation;
...
public void mark11() {
     ...
     //thisMagicNumber = markMagicNumberArray[10]/1000;
      if (randomMode == 1 ) {
         thisMarkIntensity = random(0,100);
         thisMagicNumber = random (1.6f,2.3f);
     ...
     for (int b=1;b<amplification*thisMarkIntensity*1;b++) {   //was 110
        ...
        float crevasse1cp1X = crevasse1xBegin + random (navelSize/1.3f); 
        float crevasse1cp1Y = crevasse1yBegin - random (40,navelSize/thisMagicNumber); 
        float crevasse1cp2X = crevasse1xEnd - random (navelSize/1.3f); 
        float crevasse1cp2Y = crevasse1yEnd - random (40,navelSize/thisMagicNumber); 
        strokeWeight (random (1,3));
        float distanceFromCenterBegin = dist (crevasse1xBegin,crevasse1yBegin,width/2,height/2.2f);
        ...
        if ((distanceFromCenterBegin <= (navelSize-(navelMargin+240))) && (distanceFromCenterEnd <= (navelSize-(navelMargin+250))) && (distanceFromCenterEnd < navelSize-(navelMargin+30))) {         
           ...
           for (int m = 0;m<howThick;m=m+2)  {
               ...
               crevasseAlpha=crevasseAlpha-.4f;
            }
         }
     }
      ...
      // magic number control
        if (mouseX >masterstrokeSecondLineX+403  && mouseX < masterstrokeSecondLineX+418)  {
           if (mouseY > masterstrokeSecondLineY-27 && mouseY < masterstrokeSecondLineY+24) { 
             ...
             thisMagicNumber = ((masterstrokeSecondLineY+24)-mouseY)/8.666f;
...
public void loadMasterPrescription() {
...
public void mouseReleased() {
  ...
  public void settings() {  size (1920,1156);  smooth (); }
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "Navelgazer" };
    if (passedArgs != null) {
